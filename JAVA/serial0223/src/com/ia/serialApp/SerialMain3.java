package com.ia.serialApp;
import com.ia.serialApp.util.*;
import com.ia.serialApp.handler.serialCtrl;
import com.ia.serialApp.network.*;
...
//	}
	...
	public static void main(String[] args) {
		...
		Myserial mys = new Myserial();
		SerialPort serial = mys.initSerial();
		Menu menu = new Menu();
		mys.openSerial(serial);
		
		while (true) {
			serialCtrl user = new serialCtrl();
			String mode = user.login(s);
			if (mode.equals(user.ADMIN_MODE)) {
				while (true) {
					if (menu.adminWork(s, serial)==false) {
						break;
					}
				}
			}
			else if (mode.equals(user.GUEST_MODE)) {
				while (true) {
					if (!menu.guestWork(s, serial)) {
						break;
					}
				}
			}
